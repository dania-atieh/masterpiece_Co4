...
export const addOrphanAction = (orphan) => async (dispatch) => {
  try {
    ...
    dispatch(sponsoredActions.setError(response.response?.data?.message ?? 'Something went wrong.'))
    return
  } catch (error) {
    console.log(error)
  }
...
export const updateOrphanAction = (orphan) => async (dispatch) => {
  try {
    ...
    dispatch(modalsActions.setError(response.response?.data?.message ?? 'Something went wrong.'))
    return
  } catch (error) {
    console.log(error)
  }
